...
app.use(fileUpload());
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.o0o6c.mongodb.net/${process.env.DB_NAME}?retryWrites=true&w=majority`;
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true });
client.connect(err => {
    const servicesCollection = client.db("bismillahRentCa").collection("services");
    const BookingsCollection = client.db("bismillahRentCa").collection("bookings");
    const MakeAdminCollection = client.db("bismillahRentCa").collection("admin");
    const reviewCollection = client.db("bismillahRentCa").collection("review");
